...
You can generate a strong `JWT_SECRET` using OpenSSL (macOS/Linux) or PowerShell (Windows):
...
# Windows (PowerShell)
# Run in PowerShell to generate a 32-byte base64 secret
...
### Port Already in Use
```bash
# macOS / Linux - kill process using port 3000
lsof -ti:3000 | xargs kill

# Windows (cmd): find the PID then kill
# 1) Find PID:
netstat -ano | findstr :3000
# 2) Kill it (replace <PID> with the number):
taskkill /PID <PID> /F
